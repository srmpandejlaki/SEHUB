...
const StockAdjustmentModel = {
  ...
  // Get inventory data grouped by product for adjustment
        ...
        if (selisih > 0) {
          ...
          await client.query(
            `INSERT INTO detail_barang_masuk 
             ...
             VALUES ($1, $2, $3, NULL)`,
            [insertStock.rows[0].id_barang_masuk, id_produk, selisih]
          );
          ...
          const insertDist = await client.query(
            ...
            [today, 'Sistem - Penyesuaian Stok', `Penyesuaian stok gudang${alasan ? ' - ' + alasan : ''}`]
          );
